...
export default async function Page({ params }: any) {
  ...
  return (
    <div className="px-40 py-4">
      <div
        className="flex items-center gap-10 bg-gradient-to-r from-gray-300 to-slate-800 rounded-lg p-4"
        ...
        <div className="flex-1 flex gap-4 flex-wrap">
          {categoryList.map((item) => (
            <Link
              ...
              className={
                category === item.id
                  ? "text-center bg-slate-800 rounded-2xl px-7 py-1 text-white cursor-pointer text-sm"
                  : "text-center bg-white rounded-2xl px-7 py-1 text-slate-900 cursor-pointer text-sm"
              }
            >
              <div>{item.name}</div>
            </Link>
              ...
              <Link href={`/questionBanks/${item.id}`} target="_self">
                <div className="flex gap-2 items-center justify-between">
                  <Image
                    ...
                    preview={false}
                  />
                  <div className="flex-1 flex gap-1 justify-center flex-col overflow-ellipsis overflow-hidden whitespace-nowrap">
